<template>
	<div class="queue container" v-if="reversedItems.length > 0">
		<h2 class="queue-title">Очередь на паспорт</h2>
...
const fetchItems = async () => {
	await store.dispatch("passport/fetchItems");
	items.value = store.getters["passport/getItems"];
};
...
const deleteItem = async (itemId) => {
	...
	await store.dispatch("passport/deleteItem", itemId);
	await fetchItems();
};
