<template>
	<div class="queue container" v-if="reversedItems.length > 0">
		<h2 class="queue-title">Очередь на становление президентом</h2>
...
const fetchItems = async () => {
	await store.dispatch("president/fetchItems");
	items.value = store.getters["president/getItems"];
};
...
const deleteItem = async (itemId) => {
	...
	await store.dispatch("president/deleteItem", itemId);
	await fetchItems();
};
